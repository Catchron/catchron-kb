...
2. Example Layoyut of an LVM Group
<img src="https://i.imgur.com/tX5teTP.gif" width="500"/><br>
   ...
   - Above your volume group, you can have your logical volumes such as ``lv_root``, ``lv_var``, ``lv_swap``and ``lv_home``.
...
> ``pvs`` - Lists out the physical volumens in an LVM group
> ``bgs`` - Lists out the volume groups within an LVM group
